...
def load_model():
    ...
    try:
        try:
            ...
            log.info(f"Chargement vLLM: {MODEL_PATH}")
            STATE.llm = LLM(model=MODEL_PATH, dtype="bfloat16", max_model_len=4096,gpu_memory_utilization=0.75,enforce_eager=True, disable_log_stats=True,trust_remote_code=True,max_num_seqs=128)  # Ajoutez vos args
            STATE.model_loaded = True
            log.info("vLLM chargé")
            return
        except Exception as e:
            log.error(f"vLLM échoué: {e}")
        
        # Fallback Transformers (gardez votre code existant)
        log.info("Fallback Transformers")
        import torch
...
def run_inference(req: TriageRequest) -> Dict:
    ...
    try:
        if 'vllm' in str(type(STATE.llm)).lower():  # ou check isinstance(LLM)
            ...
            raw = STATE.llm.generate(prompt, MAX_TOKENS)
        return parse_triage_response(raw)
    except Exception as e:
        log.error(f"Inférence échouée: {e}")
        return mock_triage(req.symptoms)
